namespace MoviePriceComparer.API.Infrastructure.Providers
{
    ...
    public class CinemaWorldProvider : IMovieProvider
    {
        ...
        public CinemaWorldProvider(HttpClient httpClient, ILogger<CinemaWorldProvider> logger)
        {
        ...
        public async Task<MovieDetail?> GetMovieDetailsAsync(string id)
        {
            try
            {
                ...
                var content = await response.Content.ReadAsStringAsync();
                var detail = JsonSerializer.Deserialize<MovieDetail>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                if (detail != null)
